...
void buildMPIType(int rows, int cols, bool *value, MPI_Datatype &new_type)
{
    ...
    displacements[0] = 0;
    for (int i = 1; i < rows; i++) displacements[i] = displacements[i-1] + cols;

    MPI_Type_create_struct(rows, block_lengths, displacements, typelist, &new_type);
    MPI_Type_commit(&new_type);
}

void find_dimensions(int p, int &p_rows, int &p_columns)        //using grid division;
{
    int top = sqrt(p) + 1;
...
void assign_size(int id, int p_rows, int p_cols, int &rows, int &cols, int total_rows, int total_cols)
{
    int temp_rows[p_rows];
    int temp_cols[p_cols];
    for (int i = 0; i < p_rows; i++)
    {
        temp_rows[i] = int (total_rows/(p_rows-i));
        total_rows -= temp_rows[i];
    }
    for (int i = 0; i < p_cols; i++)
    {
        temp_cols[i] = int (total_cols/(p_cols-i));
        total_cols -= temp_cols[i];
    }
    
    rows = temp_rows[id/p_cols];
    cols = temp_cols[id%p_cols];
}
void find_size(int id, int p_rows, int p_cols, int *size_rows, int *size_cols, int &rows, int &cols)
{
    int i = id/p_cols;
    int j = id%p_cols;
    rows = size_rows[i];
...
void generate_random(int size, bool * values)
{
...
int main(int argc, char *argv[])
{
    ...
    int all_rows = atoi(argv[1]);
    int all_columns = atoi(argv[2]);
    int itr = atoi(argv[3]);
    bool periodic = 1;
    periodic = atoi(argv[4]);
    if (id == 0)
    {
        cout<<"all rows = "<<all_rows<<endl;
        cout<<"all cols = "<<all_rows<<endl;
        cout<<"all itr = "<<itr<<endl;
    }
    ...
    int rows, columns;
    assign_size(id, p_rows, p_cols, rows, columns, all_rows, all_columns);


    bool *data_1 = new bool[rows*columns];
    bool *data_2 = new bool[rows*columns];
    int size = rows*columns;
    generate_random(size, data_1);
    ...
    for (int i = 0; i<itr; i++)
    {
        ...
        if (periodic) update_outer_life(rows, columns, data_this, data_nxt, top_row_rcv, bot_row_rcv, left_col_rcv, right_col_rcv, top_left_rcv, top_right_rcv, bot_left_rcv, bot_left_rcv);
    ...
    cout<<"processor "<<id<<" used "<< ((float)t)/CLOCKS_PER_SEC<<" seconds;"<<endl;
